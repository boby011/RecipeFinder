        ...
        {filteredRecipes.length > 0 ? (
          filteredRecipes.map(item => (
            <div key={item._id} className='card-container'>
              <Card className='card' onClick={() => { navigate(`/recdetails/${item._id}`) }}>        <Card.Img variant="top" src={`http://localhost:3000/uploads/${item.image}`} alt="recipe" width={'200px'} height={'200px'} />
